...
class BuildingModel:
    def __init__(self):
        # 3 Zones: Open Office (Zone 1), Meeting Room (Zone 2), Executive Suite (Zone 3)
        self.zones = ["Zone 1: Open Office", "Zone 2: Meeting Room", "Zone 3: Executive Suite"]
        ...
        self.temperatures = {
            "Zone 1: Open Office": 22.0,
            "Zone 2: Meeting Room": 22.0,
    ...
    def get_internal_gains(self, zone, occupancy, hour):
        # Occupancy heat gain: 100W per person
        q_occ = occupancy * 100.0
        
        # Equipment/Lighting load based on hour of day
        if 8 <= hour <= 18:
            q_equip = 1500.0 if zone == "Zone 1: Open Office" else (800.0 if zone == "Zone 2: Meeting Room" else 600.0)
        else:
            q_equip = 200.0
            
        return q_occ + q_equip

    def get_solar_gain(self, zone, hour, outdoor_temp):
        # Simple solar model peaking at 13:00
        if 6 <= hour <= 18:
            solar_intensity = max(0.0, math.sin((hour - 6) / 12.0 * math.pi))
            q_solar = solar_intensity * (2000.0 if zone == "Zone 1: Open Office" else 1000.0)
        else:
            q_solar = 0.0
        return q_solar

    def step(self, cooling_setpoints, heating_setpoints, occupancy, outdoor_temp, hour, dt=900):
        ...
        for z in self.zones:
            ...
            # 4. HVAC Control (Proportional-Integral style simplified to immediate steady-state response)
            # HVAC tries to keep temperature between heating and cooling setpoints.
            q_hvac = 0.0
            hvac_elec_w = 0.0
